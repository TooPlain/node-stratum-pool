...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    if (fudge !== 'kawpow') {
        ...
        if (coin.payFoundersReward === true && fudge !== 'kawpow') {
        ...
        //Vidulum VRS Support
        if (coin.VRSEnabled === true && fudge !== 'kawpow') {
...
}else
{
    this.serializeBlock = function (header_hash, nonce, mixhash) {
        ...
        header = this.serializeHeaderKawpow();
    ...
    var header_hash = this.serializeHeaderKawpow(); // 140 bytes (doesn't contain nonce or mixhash)
    // console.log("****************************");
    // console.log("Header hash sent to miners: "+header_hash.toString('hex'));
    // console.log("****************************");
    header_hash = util.reverseBuffer(util.sha256d(header_hash)).toString('hex');
