...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, soln) {
        ...
        switch (options.coin.algorithm) {
            case 'kawpow':
                kaw.hashOne(headerBuffer,nonce,blockHeight, mixOutBuf,headerHash);
                console.log("Mix hash output "+ mixHashBuf.toString('hex'));
        ...
        switch (options.coin.algorithm) {
            case 'kawpow':
                isValid = kaw.verify(headerBuffer,nonce,blockHeight,mixOutBuf,hashValueOut);
                if(!isValid) {
