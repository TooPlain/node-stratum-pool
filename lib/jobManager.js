...
var kaw = require('@mintpond/hasher-kawpow');
...
var JobManager = module.exports = function JobManager(options) {
    ...
    if (options.coin.algorithm === 'kawpow') {
        this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
            ...
            var blockHash;
            var blockHex;
            options.kawpow_validator = "getkawpowhash";
        ...
        this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, soln) {
            ...
            /** gonna get messy sorry in advance  **/
                //calculate our own header hash for kawpow, do not trust miner-given value
            var headerBufferK = job.serializeHeaderKawpow(); // 140 bytes, doesn't contain nonce or mixhash/solution
            /* var header_hash = util.reverseBuffer(util.sha256d(headerBuffer)).toString('hex');
            ...
            switch (options.coin.algorithm) {
                case 'kawpow':
                    headerHash = kaw.hashOne(headerBufferK, nonce, blockHeight, mixOutBuf, headerHash);
                    console.log("Mix hash output " + mixOutBuf.toString('hex'));
                    console.log("Header Hash output " + headerHash.toString('hex'));
                    break;
                case 'verushash':
            ...
            switch (options.coin.algorithm) {
                case 'kawpow':
                    isValid = kaw.verify(headerBufferK, nonce, blockHeight, mixOutBuf, headerHash);
                    if (!isValid) {
            ...
            if (headerBigNum.le(job.target)) {
                console.log('Block Canidate');
                blockHex = job.serializeBlock(headerBuffer, new Buffer(hashValueOut, 'hex')).toString('hex');
